...
beamWaist = 2*sigmaLambda; %[um]+

%corDimY = (0.05: 0.02: 0.150);   
corDimY = 5;  % um 2a 2 0.01
...
% corDimY = 0.15;     % um 2a 2
corDimX = 10;     % um 2a 2    10
modeNUM = 1;
...
powerConf = (1 + (sin(u + phi))^2/(2*w) + (sin(u - phi))^2/(2*wP)) / ...
                (1 + 1/(2*w) + 1/(2*wP) );
% effective width
he = corDimY(corDimIdx) + 1/sigma + 1/xi;
...
for xIdx = 1: length(y)
    ...
    if y(xIdx) > corDimY(corDimIdx)/2
        Ey(xIdx) = cos(ki*corDimY(corDimIdx)/2 - phi)*exp(-sigma*(y(xIdx) - corDimY(corDimIdx)/2));
    elseif y(xIdx) >= - corDimY(corDimIdx)/2 && y(xIdx) <= corDimY(corDimIdx)/2
        Ey(xIdx) = cos(ki*y(xIdx) - phi);
    else
        Ey(xIdx) = cos(ki*corDimY(corDimIdx)/2 + phi)*exp(xi*(y(xIdx) + corDimY(corDimIdx)/2));
    end
%     Gy(xIdx) = exp( -y(xIdx)^2/(2*sigmaLambda^2) ) / sqrt(2*pi*sigmaLambda^2);
    Gy(xIdx) = sqrt( exp( -y(xIdx)^2/(2*sigmaLambda^2) ) ); % Electric field
    Iy(xIdx) = exp( -y(xIdx)^2/(2*sigmaLambda^2) ); % Intensity profile
end

close 
% From Hunsperger book
% Direct focusing (eq. 7.3)
etaD(corDimIdx) = (trapz(Gy.*Ey))^2 / ( trapz(Gy.^2)*trapz(Ey.^2) );
...
xlabel('y[um]')
ylabel('Efield/Emax')
title(['TE0 mode profile - core ' num2str(corDimY) 'um - ref idx core ' num2str(n1)])
legend('TE0 mode profile','core edge','core edge','E beam - Gaussian approxiamtion Airy pattern','Intensity beam')
%%
%----------------------------------------------------------
% X direction
corDimX = 10;     % um 2a 2
